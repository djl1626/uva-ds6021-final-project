...
### Numeric Variables vs Temperature
...
stars2 <- gather(stars, key="predictor", value="value", Relative_Luminosity, Relative_Radius, Absolute_Magnitude)
...
### Transform Luminosity, Temperature, and Radius
...
stars <- read.csv("Data/Stars_cleanest_r.csv")
summary(stars %>% dplyr::select(Temperature, L, R, A_M))
```
...
confusionMatrix(as.factor(train_preds), as.factor(train_data$Type))
```
...
confusionMatrix(as.factor(test_preds), as.factor(test_data$Type))
```
